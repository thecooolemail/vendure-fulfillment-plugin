...
import { Between, FindOperator, In, IsNull, Not } from 'typeorm';
...
export class DeliveryRoutesService {
  ...
  async deliverableOrders(ctx: RequestContext, args: any) {
    ...
    const orders = await orderRepo.find({
      where: {
        customFields: {
          ...
          Delivery_Collection_Date: Between(
            args.options?.filter?.Delivery_Collection_Date.between.start,
            args.options?.filter?.Delivery_Collection_Date.between.end,
          ),
    ...
    const destination = await this.getPlaceDetails(
      (ctx.channel.customFields as any).google_place_id,
    ...
    for (let order of orders) {
      ...
      waypointGooglePlaceDetails.push({
        placeid: (order.customFields as any).google_place_id,
        context: `order ${order.code}`,
      });
    }
    const waypointPlaceDetails = await Promise.all(
      waypointGooglePlaceDetails.map(waypoint => this.getPlaceDetails(waypoint.placeid, waypoint.context)),
    );
    ...
    const url = encodeURI(
      `https://www.google.com/maps/dir/?api=1&origin=${origin.name}&origin_place_id=${origin.placeId}&destination=${destination.name}&destination_place_id=${destination.placeId}&waypoints=${waypoints}&waypoint_place_ids=${waypoint_place_ids}&travelmode=driving`,
    );
    return { url, orders };
  }
  ...
  private async getPlaceDetails(placeId: string, context: string): Promise<PlaceDetail> {
    ...
    const placeDetails = await axios.get(
      `https://maps.googleapis.com/maps/api/place/details/json?placeid=${placeId}&fields=name&key=${API_KEY}`,
    );
  ...
  private async getPlaceIdFromLatLon(latitude: string, longitude: string): Promise<string> {
    ...
    const queryString = encodeURI(
      `https://maps.googleapis.com/maps/api/place/textsearch/json?query=${latitude},${longitude}&inputtype=textquery&fields=place_id&key=${API_KEY}`,
    );
